...
export function deriveNodeCues(
  ...
  if (!segments.length) {
    return Array.from(
      { length: nodeCount },
      (_, index) => ((index + 1) / nodeCount) * totalMs * 0.82,
    );
  }
...
export class BootConductor {
  ...
  constructor(
    ...
    private readonly fetchNarration: () => Promise<NarrationPlan>,
  ...
  async arm() {
    ...
    try {
      this.plan = await this.fetchNarration();
    } catch {
...
async function getJson(path: string): Promise<Record<string, unknown>> {
  const response = await fetch(`${GRID_SERVICES.api}${path}`);
...
export const GRID_BOOT_NODES: BootNode[] = [
  ...
  },
  {
    ...
    probe: async () => {
      const response = await fetch(`${GRID_SERVICES.api}/api/semantic/advisors`);
      if (!response.ok) return false;
      const payload = (await response.json()) as { advisors?: Record<string, unknown> };
  ...
  },
  {
    ...
    label: "WOO ORACLE",
    probe: async () => {
      const response = await fetch(`${GRID_SERVICES.voice}/health`);
      if (!response.ok) return false;
  ...
  },
  {
    ...
    probe: async () => {
      ...
      const dcx = status.dcx as { state?: string; sealed?: boolean } | undefined;
      return dcx?.sealed === true || dcx?.state === "SEALED" || dcx?.state === "LOADED";
    },
  },
  {
    id: "perimeter",
