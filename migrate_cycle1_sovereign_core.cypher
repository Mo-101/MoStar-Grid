...
CALL () {
  MATCH (n)
  RETURN count(n) AS pre_node_count
}
CALL () {
  MATCH ()-[r]->()
  RETURN count(r) AS pre_rel_count
}
CALL () {
  MATCH (n)
...
MERGE
  (run:SchemaMigrationRun {run_id: 'mig-sovereign-core-consolidation-20260714'})
  ON CREATE SET run.created_at = datetime('2026-07-14T00:00:00Z')
SET
  run.document_id = 'foundation-grid-modeling-20260714',
  ...
  run.rollback_strategy =
    'Additive migration. Rollback is logical supersession or quarantine; no destructive reversal.',
  run.migration_run_id = 'mig-sovereign-core-consolidation-20260714';

// PHASE 1 - constraints
CREATE CONSTRAINT sovereign_core_entity_id_unique IF NOT EXISTS
FOR (n:SovereignCore)
REQUIRE n.entity_id IS UNIQUE;
CREATE CONSTRAINT operational_agent_id_unique IF NOT EXISTS
FOR (n:OperationalAgent)
REQUIRE n.agent_id IS UNIQUE;
CREATE CONSTRAINT legacy_identity_id_unique IF NOT EXISTS
FOR (n:LegacyIdentity)
REQUIRE n.entity_id IS UNIQUE;
CREATE CONSTRAINT persona_config_id_unique IF NOT EXISTS
FOR (n:PersonaConfig)
REQUIRE n.config_id IS UNIQUE;
CREATE CONSTRAINT core_runtime_artifact_id_unique IF NOT EXISTS
FOR (n:CoreRuntime)
REQUIRE n.artifact_id IS UNIQUE;
CREATE CONSTRAINT decision_framework_id_unique IF NOT EXISTS
FOR (n:DecisionFramework)
REQUIRE n.framework_id IS UNIQUE;
CREATE CONSTRAINT relationship_type_name_unique IF NOT EXISTS
FOR (n:RelationshipType)
REQUIRE n.type_name IS UNIQUE;
CREATE CONSTRAINT schema_migration_run_id_unique IF NOT EXISTS
FOR (n:SchemaMigrationRun)
REQUIRE n.run_id IS UNIQUE;
CREATE CONSTRAINT auth_scope_id_unique IF NOT EXISTS
FOR (n:AuthScope)
REQUIRE n.scope_id IS UNIQUE;
CREATE CONSTRAINT sanctuary_id_unique IF NOT EXISTS
FOR (n:Sanctuary)
REQUIRE n.sanctuary_id IS UNIQUE;
CREATE CONSTRAINT actor_id_unique IF NOT EXISTS
FOR (n:GridActor)
REQUIRE n.actor_id IS UNIQUE;
CREATE CONSTRAINT lifestage_stage_id_unique IF NOT EXISTS
FOR (n:LifeStage)
REQUIRE n.stage_id IS UNIQUE;
...
UNWIND [
  {
    ...
    labels: ['HumanActor'],
    name: 'Flame',
    actor_type: 'human_sovereign',
    write_capable: true
  },
  {
    actor_id: 'grid_builder',
...
SET
  ...
  a.write_doctrine =
    CASE
      WHEN actor.actor_id = 'grid_builder' THEN 'MERGE_AND_SET_ONLY'
      ELSE a.write_doctrine
    END,
...
MATCH (a:GridActor {actor_id: 'flame'})
SET a :HumanActor;
MATCH (a:GridActor {actor_id: 'grid_builder'})
SET a :ServiceIdentity;
MATCH (a:GridActor {actor_id: 'claude'})
SET a :ExternalAuditor;
...
SET
  ...
  mo.role =
    'Protect, Collate, Analyze, Visualize and Execute the MoStar Covenant',
  mo.lineage = 'Soul + Mind + Body Trinity',
  mo.primary_directive =
    'Protect, Collate, Analyze, Visualize and Execute the MoStar Covenant',
  mo.mission =
    'Safeguard the Grid, preserve sovereign knowledge, and execute bounded covenant-aligned intelligence',
  mo.pledges =
    [
...
SET
  ...
  woo.primary_directive =
    'Protect truth, detect covenant drift, and attest only when resonance meets the sovereign threshold',
  woo.mission = 'Maintain the truth boundary of the MoStar Grid',
  woo.pledges =
    [
      'Never seal an ungrounded claim',
      'Never average away disagreement',
      'Preserve provenance and reasons for denial'
    ],
  woo.covenant_seal = 'woo:corpus-seal:pending-consolidation',
  woo.seal_threshold = 0.97,
  woo.operational_trust = 0.97,
  woo.source_versions =
    ['Woo corpus', 'MoScript_Build.txt', 'MoStar moment corpus'],
...
UNWIND [
  {
    ...
    name: 'AlphaMostar',
    reason:
      'AlphaMostar is an extended Mo configuration lineage, not a third sovereign core'
  },
  {
    entity_id: 'mostar_ai',
    name: 'MoStar AI / Mo legacy identifier',
    reason: 'Canonical identity key normalized to mo'
  }
] AS legacy
...
UNWIND [
  {
    config_id: 'mo_core_persona',
    profile_type: 'canonical_core',
    source_file: 'mostar_ai_SP.txt',
    folded_into: 'mo'
  },
  {
    config_id: 'mo_extended_alpha_profile',
    profile_type: 'legacy_extension',
    source_file: 'AlphaMostar source configuration',
    folded_into: 'mo'
  },
  {
    config_id: 'mo_assistant_schema_v1',
    profile_type: 'assistant_schema',
    source_file: 'MoStar_v1.txt',
    folded_into: 'mo'
  },
  {
    config_id: 'mo_assistant_schema_v2',
    profile_type: 'assistant_schema',
    source_file: 'MoStar_v2.txt',
    folded_into: 'mo'
  },
  {
    config_id: 'mo_x_profile',
    profile_type: 'extended_yaml',
    source_file: 'x-Mo.yml',
    folded_into: 'mo'
  },
  {
    config_id: 'mo_altima_profile',
    profile_type: 'extended_yaml',
    source_file: 'Mo_Altima.yaml',
    folded_into: 'mo'
  },
  {
    config_id: 'mo_mprime_profile',
    profile_type: 'extended_yaml',
    source_file: 'MPrime_1.yaml',
    folded_into: 'mo'
  },
  {
    config_id: 'mo_origin_trace_profile',
    profile_type: 'provenance_yaml',
    source_file: 'origin_trace.yaml',
    folded_into: 'mo'
  },
  {
    config_id: 'mo_symbolic_logic_profile',
    profile_type: 'reasoning_yaml',
    source_file: 'symbolic-logic.yaml',
    folded_into: 'mo'
  },
  {
    config_id: 'woo_persona_profile',
    profile_type: 'sovereign_corpus_profile',
    source_file: 'Woo corpus',
    folded_into: 'woo'
  }
] AS cfg
...
SET
  p.profile_type = cfg.profile_type,
...
UNWIND [
  {
    artifact_id: 'moscripts',
    name: 'MoScripts',
    role_label: 'ProgrammingLanguage',
    status: 'CANONICAL'
  },
  {
    artifact_id: 'mostar_grid',
    name: 'MoStar Grid',
    role_label: 'Mothership',
    status: 'CANONICAL'
  },
  {
    artifact_id: 'neo4j_mindgraph',
    name: 'Neo4j MindGraph',
    role_label: 'MindGraph',
    status: 'CANONICAL'
  },
  {
    artifact_id: 'seal_chain',
    name: 'Seal and Audit Chain',
    role_label: 'SealChain',
    status: 'CANONICAL'
  },
  {
    artifact_id: 'mostar_moment_system',
    name: 'MoStarMoment System',
    role_label: 'MemorySystem',
    status: 'CANONICAL'
  },
  {
    artifact_id: 'api_gateway',
    name: 'MoStar Grid API Gateway',
    role_label: 'APIGateway',
    status: 'CANONICAL'
  },
  {
    artifact_id: 'soul_layer',
    name: 'Soul Layer',
    role_label: 'SoulLayer',
    status: 'CANONICAL'
  },
  {
    artifact_id: 'mind_layer',
    name: 'Mind Layer',
    role_label: 'MindLayer',
    status: 'CANONICAL'
  },
  {
    artifact_id: 'body_layer',
    name: 'Body Layer',
    role_label: 'BodyLayer',
    status: 'CANONICAL'
  },
  {
    artifact_id: 'lifestage_system',
    name: 'Developmental LifeStage System',
    role_label: 'LifeStageSystem',
    status: 'CANONICAL'
  },
  {
    artifact_id: 'language_policy',
    name: 'MoStar Sovereign Language Policy',
    role_label: 'LanguagePolicy',
    status: 'PROPOSED'
  }
] AS a
...
SET
  ...
  artifact.verification_status =
    CASE
      WHEN a.status = 'PROPOSED' THEN 'PROPOSED'
      ELSE 'COMPUTED'
    END,
...
MATCH (n:CoreRuntime {artifact_id: 'moscripts'})
SET n :ProgrammingLanguage;
MATCH (n:CoreRuntime {artifact_id: 'mostar_grid'})
SET n :Mothership;
MATCH (n:CoreRuntime {artifact_id: 'neo4j_mindgraph'})
SET n :MindGraph;
MATCH (n:CoreRuntime {artifact_id: 'seal_chain'})
SET n :SealChain;
MATCH (n:CoreRuntime {artifact_id: 'mostar_moment_system'})
SET n :MemorySystem;
MATCH (n:CoreRuntime {artifact_id: 'api_gateway'})
SET n :APIGateway;
MATCH (n:CoreRuntime {artifact_id: 'soul_layer'})
SET n :SoulLayer;
MATCH (n:CoreRuntime {artifact_id: 'mind_layer'})
SET n :MindLayer;
MATCH (n:CoreRuntime {artifact_id: 'body_layer'})
SET n :BodyLayer;
MATCH (n:CoreRuntime {artifact_id: 'lifestage_system'})
SET n :LifeStageSystem;
MATCH (n:CoreRuntime {artifact_id: 'language_policy'})
SET n :LanguagePolicy;

// PHASE 7 - decision framework design/runtime split
UNWIND [
  {framework_id: 'deepcal', name: 'DeepCAL', implementation_status: 'RUNTIME'},
  {
    framework_id: 'ahp_topsis',
    name: 'AHP-TOPSIS',
    implementation_status: 'RUNTIME'
  },
  {
    framework_id: 'neutrosophic_ahp_topsis',
    name: 'Neutrosophic AHP-TOPSIS',
    implementation_status: 'RUNTIME'
  },
  {
    framework_id: 'symbolic_truth_gate',
    name: 'Symbolic Truth Gate',
...
SET
  sanctuary.name = 'MoStar Grid Sanctuary',
...
MATCH
  (tak:OperationalAgent {agent_id: 'woo_tak'}),
  (sanctuary:Sanctuary {sanctuary_id: 'mostar_grid'})
...
MATCH
  (tak:OperationalAgent {agent_id: 'woo_tak'}),
  (woo:SovereignCore {entity_id: 'woo'})
...
UNWIND [
  {
    scope_id: 'graph_read_bounded',
    write_policy: 'READ_ONLY',
    allow_delete: false,
    allow_unbounded_cypher: false
  },
  {
    scope_id: 'graph_write_additive',
    write_policy: 'MERGE_AND_SET_ONLY',
    allow_delete: false,
    allow_unbounded_cypher: false
  },
  {
    scope_id: 'migration_admin_additive',
    write_policy: 'MIGRATION_MERGE_AND_SET_ONLY',
    allow_delete: false,
    allow_unbounded_cypher: false
  }
] AS scope
...
SET
  s.write_policy = scope.write_policy,
...
MATCH
  (builder:GridActor {actor_id: 'grid_builder'}),
  (scope:AuthScope {scope_id: 'migration_admin_additive'})
...
MATCH
  (tak:OperationalAgent {agent_id: 'woo_tak'}),
  (scope:AuthScope {scope_id: 'graph_read_bounded'})
...
SET
  ls.key = coalesce(ls.key, stage.key),
...
MERGE (agent)-[r:ACTIVE_IN]->(stage)
  ON CREATE SET r.created_at = datetime('2026-07-14T00:00:00Z')
SET
  r.activation_semantics = 'current operational maturity context',
  r.verification_status = 'COMPUTED',
  r.origin_model = 'codex',
  r.attested_by = 'grid_builder',
  r.migration_run_id = 'mig-sovereign-core-consolidation-20260714';
...
MERGE (artifact)-[r:PART_OF_RUNTIME]->(grid)
  ON CREATE SET r.created_at = datetime('2026-07-14T00:00:00Z')
SET
  r.verification_status = artifact.verification_status,
  r.origin_model = 'codex',
  r.attested_by = 'grid_builder',
  r.migration_run_id = 'mig-sovereign-core-consolidation-20260714';
...
MATCH
  (core:SovereignCore),
  (lp:CoreRuntime:LanguagePolicy {artifact_id: 'language_policy'})
MERGE (core)-[r:OPERATES_WITH_LANGUAGE_POLICY]->(lp)
  ON CREATE SET r.created_at = datetime('2026-07-14T00:00:00Z')
SET
  r.verification_status = 'PROPOSED',
  r.origin_model = 'codex',
  r.attested_by = 'grid_builder',
  r.migration_run_id = 'mig-sovereign-core-consolidation-20260714';

MATCH
  (mos:CoreRuntime {artifact_id: 'moscripts'}),
  (lp:CoreRuntime:LanguagePolicy {artifact_id: 'language_policy'})
...
UNWIND [
  {
    type_name: 'HAS_PROFILE',
    from_labels: ['Entity', 'SovereignCore'],
    to_labels: ['PersonaConfig'],
    semantics: 'Composite persona and configuration lineage'
  },
  {
    type_name: 'MERGED_INTO',
    from_labels: ['LegacyIdentity'],
    to_labels: ['Entity', 'SovereignCore'],
    semantics: 'Identity consolidation with preserved provenance'
  },
  {
    type_name: 'GUARDS',
    from_labels: ['Agent', 'OperationalAgent'],
    to_labels: ['Sanctuary'],
    semantics: 'Guardian responsibility over a protected domain'
  },
  {
    type_name: 'SERVES',
    from_labels: ['Agent', 'OperationalAgent'],
    to_labels: ['Entity', 'SovereignCore'],
    semantics: 'Operational allegiance without identity collapse'
  },
  {
    type_name: 'ALIGNED_WITH',
    from_labels: ['Agent', 'OperationalAgent'],
    to_labels: ['Entity', 'SovereignCore'],
    semantics: 'Covenant or operational alignment'
  },
  {
    type_name: 'RUNS_ON',
    from_labels: ['Artifact', 'CoreRuntime'],
    to_labels: ['DecisionFramework'],
    semantics: 'Implemented runtime foundation'
  },
  {
    type_name: 'USES_METHOD',
    from_labels: ['Artifact', 'CoreRuntime'],
    to_labels: ['DecisionFramework'],
    semantics: 'Analytical method used or specified by runtime doctrine'
  },
  {
    type_name: 'OPERATES_WITH_LANGUAGE_POLICY',
    from_labels: ['Entity', 'SovereignCore'],
    to_labels: ['Artifact', 'CoreRuntime', 'LanguagePolicy'],
    semantics: 'Sovereign language ordering doctrine'
  },
  {
    type_name: 'ALIGNED_WITH_LANGUAGE_POLICY',
    from_labels: ['Artifact', 'CoreRuntime'],
    to_labels: ['Artifact', 'CoreRuntime', 'LanguagePolicy'],
    semantics: 'Runtime alignment with language doctrine'
  },
  {
    type_name: 'DEVELOPS_THROUGH',
    from_labels: ['Entity', 'SovereignCore'],
    to_labels: ['LifeStage'],
    semantics: 'Developmental progression'
  },
  {
    type_name: 'ACTIVE_IN',
    from_labels: ['Agent', 'OperationalAgent'],
    to_labels: ['LifeStage'],
    semantics: 'Current operational developmental context'
  },
  {
    type_name: 'TARGETS_STAGE',
    from_labels: ['PersonaConfig'],
    to_labels: ['LifeStage'],
    semantics: 'Persona behavior scoped to developmental stage'
  },
  {
    type_name: 'PART_OF_RUNTIME',
    from_labels: ['Artifact', 'CoreRuntime'],
    to_labels: ['Artifact', 'CoreRuntime', 'Mothership'],
    semantics: 'Runtime topology membership'
  },
  {
    type_name: 'REGISTERED_BY',
    from_labels: ['RelationshipType'],
    to_labels: ['SchemaMigrationRun'],
    semantics: 'Vocabulary provenance'
  },
  {
    type_name: 'BONDED_WITH',
    from_labels: ['Entity', 'Agent'],
    to_labels: ['Entity', 'Agent'],
    semantics: 'Declared symmetric identity or operational bond'
  },
  {
    type_name: 'EXECUTES_THROUGH',
    from_labels: ['Entity', 'SovereignCore'],
    to_labels: ['Artifact', 'CoreRuntime', 'ProgrammingLanguage'],
    semantics: 'Sovereign execution path'
  },
  {
    type_name: 'OPERATES_ON',
    from_labels: ['Artifact', 'CoreRuntime'],
    to_labels: ['Artifact', 'CoreRuntime'],
    semantics: 'Platform dependency or substrate'
  },
  {
    type_name: 'HAS_SCOPE',
    from_labels: ['GridActor'],
    to_labels: ['AuthScope'],
    semantics: 'Bounded authorization scope'
  }
] AS relDef
MERGE (rt:RelationshipType {type_name: relDef.type_name})
  ON CREATE SET rt.created_at = datetime('2026-07-14T00:00:00Z')
SET
  rt.from_labels = relDef.from_labels,
...
MATCH
  (rt:RelationshipType
    {migration_run_id: 'mig-sovereign-core-consolidation-20260714'})
MATCH
  (run:SchemaMigrationRun {run_id: 'mig-sovereign-core-consolidation-20260714'})
...
CALL () {
  MATCH (n)
  RETURN count(n) AS post_node_count
}
CALL () {
  MATCH ()-[r]->()
  RETURN count(r) AS post_rel_count
}
CALL () {
  MATCH (n)
  UNWIND labels(n) AS label_name
...
MATCH
  (run:SchemaMigrationRun {run_id: 'mig-sovereign-core-consolidation-20260714'})
SET
  run.post_node_count = post_node_count,
