...
async def startup_event():
    ...
    try:
        ...
        # Pre-warm models to eliminate cold starts
        print("   - Pre-warming models...")
        try:
            # Warm up DCX1 (Soul)
            ollama.generate(
                model="Mostar/remostar-light:dcx1",
                prompt="warmup",
                keep_alive=-1  # Keep loaded indefinitely
            )
            print("   - DCX1 (Soul) pre-warmed and kept alive")
            
            # Warm up DCX2 (Body) 
            ollama.generate(
                model="Mostar/remostar-light:dcx2",
                prompt="warmup",
                keep_alive=-1
            )
            print("   - DCX2 (Body) pre-warmed and kept alive")
